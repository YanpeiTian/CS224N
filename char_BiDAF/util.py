...
def masked_average(logits, mask):
    """Take the average of `logits` over given dimension, only count where `mask` is 1."""

    masksum=torch.sum(mask,1).float()

    mask = mask.type(torch.float32)
    mask = torch.stack([mask]*logits.shape[-1],-1)
    masked_logits = mask * logits
    logitsum=torch.sum(masked_logits,1)
    masksum=torch.stack([1/masksum]*logitsum.shape[-1],-1)

    return logitsum * masksum
...
def convert_class(eval_dict, qa_id, logits_list, null_difference):
    """Convert predictions to tokens from the context.
    ...
    pred_dict = {}
    sub_dict = {}
    for qid, logit in zip(qa_id, logits_list):
        uuid = eval_dict[str(qid)]["uuid"]

        if logit[1]-logit[0]>null_difference:
            pred_dict[str(qid)] = 1
            sub_dict[uuid] = 1
        else:
            pred_dict[str(qid)] = 0
...
def eval_class(gold_dict, pred_dict):
    avna = f1 = em = total = 0
    total_truth=0
    total_predict=0
    true_positive=0
    ...
    precision = true_positive / total_predict
